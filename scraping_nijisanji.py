...
df = pd.DataFrame.from_dict(vtubers)

# 全員の年齢プロット
df_w = df[df['age'] < 500].dropna(subset=['age'])
plotter = BarPlotter(data_frame=df_w)
...
# 100歳以下のヒストグラム
df_1 = df[df['age'] < 100]
plotter = BarPlotter(data_frame=df_1)
plotter.hist('age', 100, 10)

'''
import matplotlib.pyplot as plt
# df.hist()
# pd.cut(df.age, list(range(0, 10000000001, 100000000)), right=False).value_counts().sort_index().plot.bar(color='gray')
pd.cut(getattr(df, 'age'), list(range(0, 10000000001, 100000000)), right=False).value_counts().sort_index().plot.bar(color='gray')
plt.show()
'''


'''
plt.hist(df.age.dropna(), list(range(0, 101, 5)), color='gray', rwidth=.8)
...
c = pd.cut(df.age, np.arange(0, 101, 10),
           include_lowest=True,
           right=False,
           labels = labels)
...
# print(df[df['name'].str.contains('月ノ美兎')])
